...
// Renders specified bitmap
void nokia_lcd_set_bitmap(unsigned char bitmap [])
{
	...
	for(unsigned short i = 0; i < 504; ++i)
	{
		...
		{
			/*if (pgm_read_byte(&bitmap[i]))
				nokia_lcd_set_pixel(x, y, 1);
			else
				nokia_lcd_set_pixel(x, y, 0);*/
			nokia_lcd_set_pixel(x, y + j, ((bitmap[i] >> (j)) & 0x01));
		}
		if(x < 84)
			x++;
		else
